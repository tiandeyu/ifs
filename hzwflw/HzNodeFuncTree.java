...
public class HzNodeFuncTree extends ASPPageProvider {
	   public static boolean DEBUG = Util.isDebugEnabled("ifs.hzwflw.HzNodeFuncTree");

	   public HzNodeFuncTree(ASPManager mgr, String pagePath) {
	      super(mgr, pagePath);
	   }
	   ...
	   protected String getTargetScript(String processId, String processKey, String pagePath, String nodeId)
	   {
	      ...
	      if (mgr.isEmpty(processKey)){
	    	  target = "HzNodeFuncBlank.page" + "' target='mainFrame";
	      }
	      else{
	    	  target = "HzNodeFuncSet.page?" + "P_PROCESS_KEY=" +mgr.URLEncode(processKey) + 
	         ...
	         if("TRUE".equals(getPage)){
	             ...
	             for (int i = 0; i < count; i++)
	             {
	                if("DATA".equals(buffer.getNameAt(i)))
	                {
		        		...
		        		expand_data += "&P_PAGE_PATH=" + mgr.URLEncode(pagePath);
	   ...
	   {
	      return "HZNODEFUNCTREEPROCESSTREE: Process Tree";
	   }
