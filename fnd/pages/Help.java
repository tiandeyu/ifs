...
*  File        : Help.java
*  Modified    :
*    ASP2JAVA Tool  2001-02-28  - Created Using the ASP file Help.asp
*    Chaminda O     2001-02-28  - Modifications after converting to java
*    Jacek P        2001-Mar-29 - Added handling of .page
*    Rifki R        2003-Feb-17 - Log id 1031. Fetch help base from configuration file.
*    Chandana D     2004-Jun-10 - Removed all absolute URLs.
*    Rifki R        2006-Jan-09 - Added support for "Help on Fields"
*    Rifki R        2006-Jan-10 - Replaced "common/scripts" & "fnd" with "fndweb" for documentation url
*    Rifki R        2006-Jan-20 - Fetched display names from translation file. Full name is appened only if 
*                                 display name has duplicates.
*    Rifki R        2006-Feb-01 - Explicitly set the buffer size in run() for the transaction since the 
*                                 no. of terms fetched from DB may exceed the default buffer size.
*    Prabha R       2006-Apr-20 - bug 57026 display text not displaying correctly
*    Prabha R       2006-Apr-26 -  Issues with multi frame pages
*    Prabha R       2006-Apr-26 - Bug 57490, System error occurs when basic text is null
*    Rifki  R       2006-May-17 - Bug 57749, Terms related improvements in web client.
* ----------------------------------------------------------------------------
*  
*  New Comments:
*  2008/08/12 buhilk Bug 76265, Corrected helpurl for new web feature documentations.
*  2008/04/22 buhilk Bug 72940, Added removeISOControlChars() to remove HTTP response splitting vulnerability.
*  2006/09/29 gegulk removed the usages of depricated methods getUsageID() & getDicValue()
*  2006/09/20 gegulk Bug 60579, Modified the SQL statement to enable multi language "Help on Fields"
...
public class Help extends ASPPageProvider
{
   ...
   public static boolean DEBUG = Util.isDebugEnabled("ifs.fnd.pages.Help");   
   ...
   public void run()
   {
      ...
      url  = removeISOControlChars(mgr.getQueryStringValue("url"));
      ...
      { //help on fields, generated dynamically using terms
            ...
            if(helpurl.indexOf("common/scripts")>-1)
                helpurl = mgr.replace(helpurl,"common/scripts","fndweb");
            
            //replace fnd/web/features with fndweb
            if(helpurl.indexOf("fnd/web/features")>-1)
                helpurl = mgr.replace(helpurl,"fnd/web/features","fndweb/features");
            
            helpurl = helpurl.indexOf(".asp")>-1 ? mgr.replace(helpurl,".asp.htm","Asp.htm") : mgr.replace(helpurl,".page.htm","Asp.htm");        
      }        
   ...
   {       
      ...
      appendToHTML("<br>\n");      
      appendToHTML("<p class=\"help\">"+mgr.translate("FNDPAGESSHOWONLINEDOCFORTHISPAGE: Show the IFS Online Documentation for &1this page&2","<a href="+helpurl+">","</a>"));
      appendToHTML("</p>\n");
      ...
      {         
         ...
         navcombo.append("<p class=\"help\">"+mgr.translate("FNDPAGESHELPGOTOFIELDDESC: Go to the field description for:")+"&nbsp<select name=navcombo onChange=\"goToLoc()\">\n");              
         int i = 0;
         ...
         while (it.hasNext())
         {
            ...
            bodytext.append("<a id=field_"+i+"><p class=\"help\"><b>"+tf.getLabel()+":</b></a>\n");
            bodytext.append(tf.getBasicText()+"</p>\n");
            i++;
         }          
         navcombo.append("</select></p>\n");
         bodytext.insert(0,navcombo.toString());
          
         appendToHTML(bodytext.toString());
      ...
      {
        appendToHTML("<br>\n");      
        appendToHTML("<p class=\"help\">"+mgr.translate("FNDPAGESHELPNOTERMDESCS: No term descriptions were found for this page."));
        appendToHTML("</p>\n");
      }
      endDataPresentation(false);
   }
   
   private String removeISOControlChars(String string)
   {
      String returnString = "";
